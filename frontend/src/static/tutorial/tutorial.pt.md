...
Os pesos servem para calcular o valor dos itens, das panóplias e dos builds.  
...
Características derivadas como tackle, fuga, iniciativa… são calculadas para cada item e panóplia no carregamento. Isso significa que você só deve colocar peso em uma característica se você valoriza o efeito base dela, não as características derivadas.  
...
Toda a base de dados de itens e panóplias é utilizada como referência sempre que você clica em « Calcular Melhores Itens ».

Os itens com maior valor em cada categoria aparecem na primeira coluna.  
Se um item fizer parte de uma panóplia, ele também mostra o valor “Avec Panoplie”.  
...
Efeitos especiais dos itens não entram no cálculo de valor. Pode ser necessário **travar** (lock) um item para conseguir resultados que o incluam, se boa parte do valor dele vier de um efeito especial.

As panóplias com maior pontuação (função explicada mais abaixo) aparecem na segunda coluna.  
Ao passar o mouse em uma panóplia, você vê os bônus dela e o valor que cada quantidade de peças equipadas adiciona. Os valores mostrados são incrementais: eles mostram o valor adicionado a cada peça extra.

As 20 melhores panóplias aparecem por padrão.  
Se você selecionar um item de uma panóplia que não está no top 20, essa panóplia também será mostrada no final da coluna.  
Clicando em « Selecionados » ao lado do top 20, apenas as panóplias que contêm pelo menos um item selecionado serão mostradas.
...
O objetivo é selecionar o máximo possível de itens e panóplias interessantes, mas mantendo um número de combinações razoável.

Existem duas abordagens principais:

- Usar « Seleção rápida »: adiciona todos os itens visíveis das panóplias visíveis + o item de maior valor por categoria (fora Dofus/Troféus). Mais rápido, mas potencialmente menos otimizado.
- Selecionar manualmente os itens e panóplias que parecem relevantes.

Depois disso, verifique o total de combinações.  
Se estiver muito alto, remova algumas panóplias piores. Tente manter abaixo de 100 milhões em um primeiro teste.  
...
Se ele não respeitar uma condição de: Nível ou bônus de panóplia < 3 → a condição aparece com fundo vermelho e valor = 0.  
...
As tooltips de itens/panóplias passam a ter rolagem própria quando ficam muito grandes. A rolagem então age na tooltip, não na página.
...
Para calcular o valor que uma panóplia traz para um item, a aplicação calcula o valor de cada bônus da panóplia e divide pelo número de itens necessários para obter esse bônus. O melhor valor por item é o que conta.  
Esse valor aparece como « Melhor valor per item » e é somado ao valor de cada item da panóplia.

Na maioria dos casos, esse valor máximo corresponde à panóplia completa, mas às vezes um patamar intermediário pode ter uma média melhor.

## Score

Para calcular as melhores panóplias, é usada uma função de pontuação (score). Não é obrigatório entender, mas aqui vai a explicação:

Para cada item da panóplia, subtrai-se o valor dele do valor do melhor item da mesma categoria. Isso representa o valor perdido por não usar o melhor item possível. Essas perdas são ordenadas da maior para a menor.  
Para cada bônus de panóplia, calcula-se o valor do bônus menos a soma das perdas dos X primeiros itens necessários, e depois divide-se pelo número total de itens da panóplia. O melhor valor obtido é o score final.

A ideia: ordenar as panóplias pela **melhor** contribuição possível delas, levando em conta a perda potencial por não usar os melhores itens isolados de cada categoria.

Um score negativo não significa necessariamente que a panóplia é ruim: ela pode ser essencial para cumprir um parâmetro Min (geralmente PA), para evitar atingir um parâmetro Max, ou para satisfazer alguma condição.

Um ponto fraco dessa função de score é que o melhor item de uma categoria pode ser caro demais ou simplesmente não desejado. Nesses casos, uma panóplia que inclui um item dessa categoria pode ser subavaliada, mesmo sendo relevante para o seu caso.

Então não trate esse score como uma verdade absoluta. Pode ser necessário adicionar panóplias com score mais baixo, ou remover panóplias que não fazem sentido para a busca que você está fazendo.

## Panóplias parcialmente selecionáveis

Se uma panóplia tiver um item não selecionável porque:
...
Se só uma peça (ou nenhuma) for selecionável, o score vira −Infinito, e a panóplia não é mostrada.

Depois de travar um item, pode ser interessante recalcular os melhores itens para obter scores atualizados. Isso vai te dizer: “considerando os itens que você travou, aqui estão os melhores itens e panóplias que se adaptam aos espaços restantes”.  
Você pode usar essa lógica para montar um build travando aos poucos alguns itens ou panóplias e recalculando os melhores itens a cada etapa.

## Seleção rápida

A « Seleção rápida » adiciona todos os itens visíveis das panóplias visíveis. Se um item não puder ser selecionado (por causa de nível ou Min incompatível), ele não é adicionado.

Você pode ajustar a quantidade de panóplias mostradas (Top 20) para adicionar mais ou menos itens.

A seleção rápida também adiciona o item de maior valor em cada categoria (2 nos anéis).  
A lógica é: se esse item é o melhor da categoria fora de panóplia, escolher outro costuma significar menos valor. Claro que se as características de um item interagem com valores capados, isso pode mudar bastante as coisas. Mas como estamos falando de uma seleção **rápida**, é melhor verificar que itens foram adicionados e incluir outros manualmente se você quiser resultados ainda melhores.

## Panoplie Momore

A panoplie Momore é a única panóplia que tem condições próprias. Ela funciona como as condições dos itens:  
Se os seus parâmetros Min de PM / Alcance / Invocações forem incompatíveis com o bônus da panóplia, as funções de score e valor por item levam isso em conta e excluem as combinações afetadas.

Caso contrário, o valor final do build será capado de acordo com o bônus da panóplia.  
Se você quiser comparar builds que usam essa panóplia, é recomendável colocar um peso baixo ou zero nas características capadas por ela.
...
Por exemplo, você pode travar 5 Dofus / Troféus e deixar Turbulent (+1 PA, –1 PM) e mais um outro troféu útil para o seu build destravados. Isso gera apenas 2 combinações, permitindo comparar facilmente resultados com e sem Turbulent dentro da mesma busca.

## Troféus com bônus de panóplia < 3

Se você selecionar pelo menos um Troféu que tenha como pré-requisito “bônus de panóplia < 3”, então são criados dois grupos de combinações:

- **Grupo 1**: somente Dofus/Troféus “clássicos”
- **Grupo 2**: os Troféus “bônus pano < 3” têm prioridade

A ideia é comparar builds que têm a condição “bônus de panóplia < 3” com builds que não têm.  
Um build com uma condição tão restritiva tende a ser menos otimizado na maioria dos casos. Então faz sentido colocar o máximo possível desses Troféus “bônus pano < 3” em um build, já que eles têm mais valor.

O primeiro grupo “clássico” corresponde às combinações de Dofus / Troféus **sem** os Troféus bônus de panóplia < 3. Ele pode ficar vazio se você só tiver esse tipo de troféu selecionado.

O segundo grupo, com bônus de panóplia < 3, é criado assim:

1. Primeiro, seleciona os Dofus / Troféus “clássicos” travados
2. Depois:
    - Se houver tantos ou mais Troféus bônus de panóplia < 3 quanto espaços restantes → cria combinações **apenas** com eles → fim
    - Senão, preenche todos os espaços restantes com os Troféus bônus de panóplia < 3
3. Se ainda sobrarem espaços e existirem Troféus clássicos não travados → cria combinações com eles nesses espaços restantes → fim

A forma mais simples de lidar com esse agrupamento é ter só uma combinação (6 itens) para cada grupo.  
Para isso, selecione 6 Dofus / Troféus “clássicos”, trave os que você quer em todos os casos e depois selecione [6 − Dofus / Troféus clássicos travados] Troféus bônus de panóplia < 3.  
Por exemplo: se você seleciona 6 Troféus clássicos, dos quais 2 estão travados, adicione 4 Troféus bônus de panóplia < 3.
...
Esse agrupamento **não** acontece se você travar pelo menos um Troféu bônus de panóplia < 3.  
Nesse cenário, todas as combinações são calculadas normalmente. Então, se você só quiser explorar builds com essa condição, travar um único Troféu bônus de panóplia < 3 já resolve. Pode ser mais simples, especialmente se esse esquema de agrupamento parecer confuso (o que é bem compreensível).
...
Por exemplo, se você usar somente o feitiço Sacrier “Décimation”: ele causa de 29 a 32 de dano Terra, em média 30,5. O ratio seria 100 / 30,5 ≈ 3,3.  
...
Por exemplo, a Fúria de Gargandyas exige pelo menos 5000 de Vitalidade no nível 200.  
...
Depois que você tiver pesos que parecem coerentes, pode rodar um cálculo de combinações maior, adicionando bem mais itens e panóplias, caso exista alguma combinação “escondida” muito interessante.  
Tenha em mente que quanto menor o valor dos itens/panóplias adicionais, menor a chance de encontrar um build realmente melhor por causa deles.

O cálculo das combinações é bem agressivamente otimizado, então, se você estiver disposto a esperar, pode dar uma quantidade grande de itens e panóplias para a aplicação testar.

Se chegar em um ponto em que o cálculo demore demais, você pode dividir a busca em sub-buscas.  
É quase garantido que os melhores builds de uma busca vão ter pelo menos uma panóplia.  
Você pode então travar uma por uma as melhores panóplias para o seu build e rodar buscas separadas.  
Isso exclui todas as combinações sem panóplia no build final, mas dá mais trabalho.  
...
Você também pode recalcular os melhores itens quando travar panóplias diferentes, obtendo uma seleção mais precisa como explicado antes.
...
Eles têm um pouquinho menos valor que os mascotes anteriores, mas o PA que dão pode fazer uma diferença enorme e eles oferecem características bem flexíveis.  
